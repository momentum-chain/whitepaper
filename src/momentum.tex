...
For each timestamp there exists an order of addresses that determines the priority for each address to produce the next block.

\subsection{Rank}

Each block has a $rank$ and $total rank$ value. The $rank$ value specifies its rank in the priority list during the timestamp of block production.
