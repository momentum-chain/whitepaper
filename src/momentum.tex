...
The goal of the nodes is to work together to build a chain of valid blocks. A block is valid if:
...
For each timestamp there exists a global order of all accounts that hold Momentum. This order is calculated using a weighting function that takes
 a timestamp and the set of all accounts as input, and orders them using a weighted random
